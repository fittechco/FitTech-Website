...
export default function ContactUs({ myRef }: { myRef: RefObject<HTMLDivElement> }) {
  ...
  useEffect(() => {
    ...
    const handleHeight = () => {
      textAreaElement.style.height = textAreaRef.current?.scrollHeight + "px"
    }
    textAreaRef.current! && textAreaElement.addEventListener("keyup", handleHeight)
    return () => {
      textAreaElement.removeEventListener("keyup", handleHeight)
    }
  }, [])
