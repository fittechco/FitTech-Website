...
export default function Hero() {
  ...
  return (
    ...
    <section ref={heroRef} className="hero flex items-center / w-full h-full  bg-white  relative overflow-x-hidden z-10 px-6 md:px-20 min-h-screen">
      ...
      <Polygon className={`-z-[1] absolute top-[8%]  left-[60%] lg:left-[45%] transition-all duration-500 ease-in-out delay-200  ${heroVisible? "scale-50 lg:scale-75 ":"scale-0"}`} />
      <Polygon className={`-z-[1] absolute top-[70%] left-[10%] lg:left-[10%] transition-all duration-500 ease-in-out delay-[250] ${heroVisible? "scale-[60%]":"scale-0"} `} />
      <Polygon className={`-z-[1] absolute top-[50%] left-[30%] lg:left-[15%] transition-all duration-500 ease-in-out delay-300  ${heroVisible? "scale-[50%]":"scale-0"}`} />
      ...
      <Polygon className={`-z-[1] absolute top-[40%] left-[70%]  md:block lg:hidden transition-all duration-500 ease-in-out delay-[400] ${heroVisible? "scale-50 md:scale-75":"scale-0"}`} />
      <PolysAnimation visible={heroVisible} />
      <div className="heroInfo / flex flex-col gap-6 md:gap-8 / z-10 md:mt-6 antialiased ">
        <MyText type={"h2"} className={`text-thirdColor font-bold  md:max-w-md lg:max-w-3xl transition-all duration-500 ease-in-out ${heroVisible? "translate-x-0 opacity-100" : "-translate-x-1/4 opacity-0"}`}>
          Fitness meets technology: Solutions for the modern fitness industry
        </MyText>
        <MyText type={"p"} className={`text-thirdColor3 antialiased  md:max-w-lg transition-all ease-in-out duration-500 delay-200 ${heroVisible? "translate-x-0 opacity-100" : "-translate-x-1/4 opacity-0"}`}>
        ...
        <MyButton className={`transition-all delay-[400ms] duration-500 ease-in-out ${heroVisible? "translate-x-0 opacity-100" : "-translate-x-1/4 opacity-0"}`} text={"Contact Us"} type={""} form={""} />
      </div>
    </section>
  );
}
